...
#### `How to` guide
---
First of all, please make sure that you have at least 1 GPU on hand (16GB GPU memory is required to run the device model). You are recommended to set up the Python environment with [Anaconda](https://www.anaconda.com/products/distribution#linux).
```shell
sh Anaconda3-<yyyy.mm>-Linux-x86_64.sh
```
You need to specify the location for installing Anaconda after running the command above. Ensure you have permission and enough space in the selected installing location.

After installing Anaconda, you need to create the environment for DeepQueueNet first. To create the Anaconda environment, you can simply run the command:

```shell
conda env create -f env.yml
```

You can also create the environment with the name specified by you with:

```shell
conda env create -f env.yml -n <name>
```

Then, you can activate the Anaconda environment before running the code of DeepQueueNet with:

```shell
conda activate dqn_env_py36 # replace dqn_env_py36 with <name> if you specify the name
```

So far, all the environment preparations for DeepQueueNet are finished. 

The following part will guide you through all the sections of DeepQueueNet implementation.
