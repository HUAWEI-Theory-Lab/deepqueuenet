...
class deepQueueNet(trace2Samples):
    ...
    def _loop_size(self): 
        ...
        file_list=glob.glob('./data/{}/_tfrecords/*.tfrecords'.format(self.config.modelname))
    ...
    def parse(self, serialized):
        with tf.device("/cpu:0"):
            with tf.name_scope('parse'): 
                ...
                for k in ['fet','label']: features[k] = tf.sparse_tensor_to_dense(features[k])
    ...
    def tfrecords_input_fn(self, shuffle_buf=1000000):
        ...
        if shuffle_buf: 
            ds = ds.apply(tf.contrib.data.shuffle_and_repeat(shuffle_buf, count=self.config.epochs))
        ...
        X=tf.reshape(X['fet'], [-1, self.config.TIME_STEPS, len(self.fet_cols)])
        return X, y
    ...
    def _create_one_cell(self, layer): 
        """lstm: nn layer"""
        cell=tf.contrib.rnn.LSTMCell(self.config.lstm_params['cell_neurons'][layer], forget_bias=0,  state_is_tuple=True)
        if self.config.lstm_params['keep_prob']==1.0:
            return cell 
        else:
            return tf.contrib.rnn.DropoutWrapper(cell, output_keep_prob=self.config.lstm_params['keep_prob']) 
    ...
    def build_and_training(self):
        ...
        with lstm_graph.as_default(): 
            ...
            with tf.variable_scope('lstm1', initializer= tf.orthogonal_initializer(),  reuse=None):
                multi_cell=tf.contrib.rnn.MultiRNNCell([self._create_one_cell(_)
...
#                 val= val[0] + val[1]
            ...
            rnn1=tf.reshape(val, [-1, self.config.TIME_STEPS, self.config.lstm_params['cell_neurons'][-1]])
            for mul_head in range(self.config.mul_head):
                with tf.variable_scope('head{}'.format(mul_head+1), initializer= tf.orthogonal_initializer(),  reuse=None):
                    V=tf.layers.dense(inputs=rnn1, units=self.config.att, kernel_initializer=initializer)
                    K=tf.layers.dense(inputs=rnn1, units=self.config.att, kernel_initializer=initializer)
                    Q=tf.layers.dense(inputs=rnn1, units=self.config.att, kernel_initializer=initializer)
                    score=tf.matmul(Q, tf.transpose(K, [0,2,1]))/tf.sqrt(self.config.att)
                    ...
                    if mul_head==0:
                        ...
                        Z=tf.concat([Z, z], -1)
            O=tf.layers.dense(inputs=Z, units=self.config.mul_head_output_nodes, kernel_initializer=initializer)
            dense_outputs=tf.reshape(O, [-1, self.config.TIME_STEPS, self.config.mul_head_output_nodes])
            ...
            #decoder layer
            with tf.variable_scope('lstm2', initializer= tf.orthogonal_initializer(), reuse=None):
                multi_cell2=tf.contrib.rnn.MultiRNNCell([self._create_one_cell(_) 
                                                         for _ in range(self.config.lstm_params['layer'])], 
                                                        state_is_tuple=True)
                val2, __=tf.nn.dynamic_rnn(multi_cell2, dense_outputs, dtype=tf.float32)
...
#                 val2= val2[0] + val2[1]
            ...
            #output layer  
            rnn2 = tf.reshape(val2, [-1, self.config.lstm_params['cell_neurons'][-1]]) 
            stacked_outputs = tf.layers.dense(rnn2, 
                                              self.config.n_outputs,
                                              kernel_regularizer=tf.contrib.layers.l2_regularizer(self.config.l2), 
                                              kernel_initializer=initializer)
            outputs=tf.reshape(stacked_outputs, [-1, self.config.TIME_STEPS, self.config.n_outputs])
            outputs=outputs[:, self.config.TIME_STEPS-1, :] #keep only the last output of the seq
        ...
        with tf.Session(graph=lstm_graph) as sess: 
            ...
            print(mat_c.format("iteration (in k)", "MSE- train/test1/test2", "time used"))
            while True:
                try:
                    _, loss_value=sess.run([train_op,loss])
                    if it%1000==0:
                        mse_test1=loss.eval(feed_dict={X:self.x_test1, y:y_test1})
                        mse_test2=loss.eval(feed_dict={X:self.x_test2, y:y_test2})
                        mse_train=loss.eval(feed_dict={X:self.x_train, y:y_train})
                        ...
                        if mse_test1+mse_test2<max_mse:
                            ...
                            saver.save(sess,'./save/{}/model.ckpt'.format(self.config.modelname), global_step=it//1000)
                        print(mat_t.format(it//1000, 
