...
class INFER(Init_TOPO):
    ...
    def run(self):
        ...
        def build_and_clean(device, my_result):
            for i in range(4*device, 4*(device+1)): my_result[i]=pd.DataFrame(columns=self.used_cols)
        with Manager() as MG:
            my_result=MG.dict() 
            ...
            def pod_infer(device, my_result, my_link):
                ...
                with tf.Session() as sess:
                    ...
                    #IRSA
                    build_and_clean(device, my_result)
                    for i in range(12+2*device, 14+2*device): self.trace_upd('edge', i, sess, outputs, X, my_result, my_link)
                    build_and_clean(device, my_result)
                    for i in range(4+2*device, 6+2*device): self.trace_upd('agg', i, sess, outputs, X, my_result, my_link)
                    self.trace_upd('core', device, sess, outputs, X,my_result, my_link) 
                    for i in range(4+2*device, 6+2*device): self.trace_upd('agg', i, sess, outputs, X, my_result, my_link)
                    for i in range(12+2*device, 14+2*device): self.trace_upd('edge', i, sess, outputs, X, my_result, my_link)
