...
def buildfolder(root):
    for name in ['_hdf', '_hdf/train', '_hdf/test1', '_hdf/test2', '_scaler', '_tfrecords', '_error']:
...
class trace2Samples:
    ...
    def multi_process(self, func, FILES, args=None):
        it=0
        while True: 
            files=FILES[it*self.config.no_process: (it+1)*self.config.no_process]
            if len(files)>0:
                threads = []
                for file in files:
                    ARGS=list(args)
    ...
    def fet_input(self):
        ...
        def gettraffic(dst_folder, my_fet, file):
            ...
            for i in range(self.config.no_of_port): df['TI%i' %i]=LOAD[i]
            ...
            if self.config.no_of_buffer>1:
                for i in range(self.config.no_of_buffer):
                    t=df[df['priority']==i]['timestamp (sec)'].diff().rename('inter_arr{}'.format(i)).to_frame()
                    df=df.join(t)
            ...
            df[fet_cols+self.target].fillna(method='ffill').dropna().to_csv('{}/{}'.format(dst_folder, filename), index=False)
        ...
        with Manager() as MG: 
            ...
            for mode in ['train', 'test']:
                file_dir='./data/{}/_traces/_{}'.format(self.config.modelname, mode)
                FILES=[]
                ...
                dst_folder= './data/{}/_traces/{}'.format(self.config.modelname, mode)
                if os.path.exists(dst_folder):  
                    shutil.rmtree(dst_folder) 
                os.makedirs(dst_folder)
                self.multi_process(gettraffic, FILES, args=(dst_folder, my_fet))
    ...
    def _2hdf(self):
        ...
        def split_2hdf(mode, file):
            ...
            loc=np.random.choice(np.arange(len(y)), np.max([np.min([14000, len(y)]), int(len(y)*0.15)]),  replace=False)  
            x=x[loc]
            y=y[loc]
            if mode=='train':
                x_train, x_test, y_train, y_test = train_test_split(x, y, test_size=self.config.test_size, shuffle=True)
                self.write_hdf('./data/{}/_hdf/train/{}.h5'.format(self.config.modelname, key), x_train, y_train)
                self.write_hdf('./data/{}/_hdf/test1/{}.h5'.format(self.config.modelname, key), x_test, y_test)
            else:
                self.write_hdf('./data/{}/_hdf/test2/{}.h5'.format(self.config.modelname, key), x, y)
    ...
    def _min_max(self):  
        ...
        for i, file in enumerate(glob.glob(src)):
            ...
            ymin=pd.Series(np.array([y.min(axis=0).min(axis=0)]).flatten(), index=self.target)
            ymax=pd.Series(np.array([y.max(axis=0).max(axis=0)]).flatten(), index=self.target)
            if i==0:
                self.x_MIN=xmin
                self.x_MAX=xmax
                self.y_MIN=ymin
    ...
    def load_scaler(self):
        ...
        self.x_MIN, self.x_MAX, self.y_MIN, self.y_MAX, self.fet_cols, self.target=load_scaler(folder)
    ...
    def model_input(self):  
        ...
        def inner_op(mode, file, h):
            ...
            #normalization
            x=(x-self.x_MIN.values)/(self.x_MAX.values-self.x_MIN.values)
            y=(y-self.y_MIN.values)/(self.y_MAX.values-self.y_MIN.values)
            
            #save it in .tfrecords
            if mode=='train':
                writer = tf.python_io.TFRecordWriter('./data/{}/_tfrecords/{}.tfrecords'.format(self.config.modelname, key))
                
                """sampling part of them for eval. during the training phase.""" 
                loc=np.random.choice(np.arange(len(y)), int(len(y)*self.config.sub_rt), replace=False)
                x_valid=x[loc]
                y_valid=y[loc]
                self.write_hdf2(h, key,x_valid,y_valid)
                for a,d in zip(x,y):
                    example = tf.train.Example(features=tf.train.Features(feature={
                        'fet':tf.train.Feature(float_list=tf.train.FloatList(value=a.reshape(1,-1)[0])),
                        'label':tf.train.Feature(float_list=tf.train.FloatList(value=d))}))
                    writer.write(example.SerializeToString())
                writer.close()
            elif mode=='test1':
                ratio=(1.-self.config.test_size)/self.config.test_size
                loc=np.random.choice(np.arange(len(y)), int(len(y)*self.config.sub_rt*ratio), replace=False)
                x=x[loc]
                y=y[loc]
        ...
        for mode in ['train', 'test1', 'test2']:
            ...
            with h5py.File('{}/{}.h5'.format(folder, mode), mode='w') as h:  
                for file in glob.glob(src): inner_op(mode, file, h)
    ...
    def merge_sample(self, task):
        ...
        with h5py.File('./data/{}/_hdf/{}.h5'.format(self.config.modelname, task), 'r+') as hdf:
            for i,key in enumerate([key[:-2] for key in hdf.keys() if '_x' in key]):
                if i==0:
                    x=hdf['{}_x'.format(key)][:]
                    y=hdf['{}_y'.format(key)][:]
                else:
